  ...
  mutation anmeldung(
    ...
    createAnmeldung(
      data: {
        ...
        veranstaltung: { connect: { id: $veranstaltungId } }
      }
    ) {
      id
    }
...
export const handler: Handler = async (event) => {
  ...
  const createAnmeldungResponse = await graphQLClient.request(
    createMutation,
    variables,
  )
  const {
    createAnmeldung: { id },
  } = createAnmeldungResponse
  const publishAnmeldungResponse = await graphQLClient.request(
    publishMutation,
    { id },
  )
  const { publishAnmeldung } = publishAnmeldungResponse
  const emailTemplate = isAvailable(veranstaltung)
    ? templates.register
    : templates.waitingList
