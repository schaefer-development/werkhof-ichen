      ...
      <v-col cols="12" xs="12" sm="8" md="8" lg="8" xl="8">
        <v-card rounded="0" flat class="ichen_beige">
          ...
          <div v-if="success" class="pt-8 pr-3 pb-2 pl-3">
            ...
            <v-card-text>
              <p>
                <strong>{{ veranstaltung.Titel }} am {{ veranstaltung.Datum | formatDate }} Uhr</strong>
              </p>
              <p>
                Sie erhalten in den nächsten Minuten eine Bestätigung Ihrer Anmeldung mit allen weiteren Details per E-Mail.
                Wenn Sie keine Nachricht erhalten haben, schauen Sie auch bitte einmal in Ihrem Spamordner nach.
              </p>
              <p>
                Falls Sie keine Bestätigung erhalten haben, nehmen Sie bitte Kontakt mit mir auf:
          ...
          <div v-else class="pl-3 pr-3">
            ...
            <v-card-text>
              ...
              <v-form ref="form" v-model="valid" class="pt-4 pb-12">
                <v-text-field v-model="name" outlined :rules="requiredRules" required>
                  <template v-slot:label>
                    <div class="jos_label">Vor- und Nachname</div>
                  </template>
                </v-text-field>

                <v-text-field v-model="email_adresse" outlined :rules="emailRules" required>
                  <template v-slot:label>
                    <div class="jos_label">E-Mailadresse</div>
                  </template>
                </v-text-field>
                <v-text-field v-model="telefon_nummer" outlined :rules="requiredRules">
                  <template v-slot:label>
                ...
                <v-checkbox v-model="agb" :rules="agbRules" required>
                  <template v-slot:label>
                    <div>
                      <div class="jos_label">
                        Ich habe die
                        <nuxt-link to="/anfertigungen/leder">AGB</nuxt-link>&nbsp;gelesen und akzeptiere sie
                      </div>
                    </div>
                  </template>
                </v-checkbox>
